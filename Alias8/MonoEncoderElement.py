	...
	def connect_to(self, parameter):
		...
		if(str(parameter.name) == str('Track Volume')):		#checks to see if parameter is track volume
			if(parameter.canonical_parent.canonical_parent.has_audio_output is False):		#checks to see if track has audio output
				for device in parameter.canonical_parent.canonical_parent.devices:
					if(str(device.class_name)==str('MidiVelocity')):	#if not, looks for velicty as first plugin
						assignment = device.parameters[6]				#if found, assigns fader to its 'outhi' parameter
