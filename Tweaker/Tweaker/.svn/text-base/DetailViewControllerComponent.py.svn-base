...
class DetailViewControllerComponent(ControlSurfaceComponent):
	...
	def _nav_value(self, value, sender):
		assert ((sender != None) and (sender in (self._left_button,
												 self._right_button)))
		#self._script.log_message('nav_value' + str(sender) + str(value))
		if (self.is_enabled() and (not self._shift_pressed) and (value > 0)):
			#if self.song().view.selected_track == self._channelstrip._track:
			...
			device_number = -1
			#self._script.log_message('len devs ' + str(len(track.devices)))
			for index in range(len(track.devices)):
				#self._script.log_message(str(track.devices[index]) + str(selected_device))
				if track.devices[index] == selected_device:
					device_number = index
					break
			#self._script.log_message('dev num' + str(device_number))
			#device_number = track.devices.index(selected_device)
			if device_number > -1:
				if sender == self._right_button:
					device_number += 1
				elif sender == self._left_button:
					device_number -= 1
				if device_number in range(len(track.devices)):
					self._channelstrip._device_component.set_device(track.devices[device_number])
					#track.view.selected_device = track.devices[device_number]
